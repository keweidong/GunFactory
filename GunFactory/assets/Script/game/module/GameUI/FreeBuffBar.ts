...
@ccclass
export default class FreeBuffBar extends cc.Component implements IShareAndAd {
    // @property(cc.Label)
    ...
    */
    ...
    public init(itemId: number) {
        let item: BuffItem = this.item = App.ControllerManager.applyFunc(ControllerConst.Item, BagConst.GET_ITEM, itemId)
        this.buffConf = App.ConfigManager.getConfig("BuffDataManager").getData(item.data.agrs4);
        this._btnBox.descLab.string =
            // this._buffBox.descLab.string =
            GameText.getText(lang.free_buff) + (item.data.agrs2 / 100 + 1);
        this._btnBox.timeLab.string = item.data.agrs0 / 60 + GameText.getText(lang.common_minute);//"分钟";
        App.ControllerManager.applyFunc(ControllerConst.AD, ADConst.REGISTER_ITEM, AdType.FREE_BUFF, this);
    }
    protected onTouchBtn() {
        if (this._state == 1) {
            App.ControllerManager.applyFunc(ControllerConst.GameUI, GameUIConst.GET_FREE_BUFF);
        }
    }
    ...
    */
    ...
    public setData(buff: BuffData) {
        if (buff) {
            ...
            this._btnBox.descLab.string = GameText.getText(lang.free_buff) + (this.item.data.agrs2 / 100 + 1);
