/*
 * @Author: He 
 * @Date: 2020-01-21 11:48:46 
 * @Last Modified by: He
 * @Last Modified time: 2020-04-16 16:45:02
 */
/**
 * a*算法
 */
export class SilzAstar {
     ...
     * @param    mapdata        地图数据
     */
    public constructor(mapdata: number[][], protected cellWidth: number = 64, protected cellHeight: number = 64) {
    ...
    public find(xnow: number, ynow: number, xpos: number, ypos: number): PathNode[] {
        ...
        // xpos = Math.floor(xpos / this.cellWidth);
        // ypos = Math.floor(ypos / this.cellHeight);
        xpos = Math.min(xpos, this._grid.numCols - 1);
        ypos = Math.min(ypos, this._grid.numRows - 1);
        this._grid.setEndNode(xpos, ypos); //1

        // xnow = Math.floor(xnow / this.cellWidth);
        // ynow = Math.floor(ynow / this.cellHeight);
...
export class BinaryHeap {
    ...
    public ins(value: PathNode): void {
...
export class Grid {
    ...
    private initNodeLink(node: PathNode, type: number): void {
        ...
        for (var i: number = startX; i <= endX; i++) {
            for (var j: number = startY; j <= endY; j++) {
                ...
                if (type != 2 && i != node.x && j != node.y) {//四个斜角
    ...
    public updateNodeLinks(x: number, y: number, width: number, height: number) {
        var startX: number = Math.max(0, x - 1);
        var endX: number = Math.min(this.numCols - 1, x + width);
        var startY: number = Math.max(0, y - 1);
        var endY: number = Math.min(this.numRows - 1, y + height);
        for (var i: number = startX; i <= endX; i++) {
            for (var j: number = startY; j <= endY; j++) {
                this.initNodeLink(this._nodes[i][j], this.type);
            }
        }
    }
...
export class PathNode {
    ...
    public static create(x: number, y: number): PathNode {
        let obj = ObjectPool.pop("PathNode");
        obj.x = x;
        obj.y = y;
        return obj;
    }
}
ObjectPool.registerClass(PathNode, "PathNode");
