...
window.boot = function (projectJsPath) {
    ...
    // var bundledScript = settings.debug ? 'src/project.dev.cc809.js' : 'src/project.js';
    if (jsList) {
...
// window.boot();
window.boot(getRealpath(window._CCSettings.debug ? 'src/project.dev.js' : 'src/project.js'));