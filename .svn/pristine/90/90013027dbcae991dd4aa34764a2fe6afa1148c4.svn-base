 ...
 * 穿山甲广告激励广告
 */
export class TopOnRewardVideoAd implements IAdObject {
    /**广告加载完毕调用 */
     ...
     * 是否支持多个id切换
     */
    protected isMultiId: boolean = false;
    protected idIndex: number = 0;
    /**广告展示完毕调用 */
    ...
    constructor(id: string) {
        this.id = id;
        // this.idIndex = 0;
        // this.idIndex = App.RandomUtils.limitInteger(0, id.length - 1);
    }
    /**
     * 拉取激励广告成功回调
     */
    ...
    private addADloadFailCallBack(error) {
        Log.trace("addADloadFailCallBack error", error)
        Log.trace("加载广告失败")
    ...
    public init() {
        ...
        if (cc.sys.platform == cc.sys.ANDROID) {
            let result = jsb.reflection.callStaticMethod(this._classPath, "isMultiRewardId", "()Z");
            Log.trace("result:", result)
            if (result) {
                this.isMultiId = result;
            }
        } else {

        }
        //    console.log("isMultiRewardId:", ) ;
        // Log.trace("initAd2");
    }
    ...
    public initAd() {
        ...
        if (cc.sys.platform == cc.sys.ANDROID) {
            Log.trace("topon激励广告initAd")
            jsb.reflection.callStaticMethod(this._classPath, "initRewardVideoAd", "()V");
        } else {
    ...
    *  安装完成
    */
    protected onInstalled(appName: string) {
        // App.ControllerManager.applyFunc(ControllerConst.Install, InstallWelfareConst.INSTALL_APP, appName);
    }
    /**
     * 下载完成
     */
    ...
    public load() {
        ...
        if (cc.sys.platform == cc.sys.ANDROID) {
            if (this.isMultiId) {//支持多个id切换的
                var result = jsb.reflection.callStaticMethod(this._classPath, "loadRewardVideoAd", "(Ljava/lang/String;)V");
            } else {
    ...
    // public getId() {
    //     this.idIndex++;
    //     if (this.idIndex === this.id.length) {
    //         this.idIndex = 0;
    //     }
    //     Log.warn("激励广告id", this.id[this.idIndex], this.idIndex);
    //     Log.trace("激励广告id列表", this.id);
    ...
    public show() {
        ...
        if (cc.sys.platform == cc.sys.ANDROID) {
            ...
            cc.log("ios播放广告")
            Log.trace("ios播放广告")
            var result = jsb.reflection.callStaticMethod("BUDRewardedVideoAdViewController", "showRewardVideoAd:", this.id);
        }
    }
}